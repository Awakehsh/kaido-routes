...
EXPECTED_CHECKPOINT_IDS = {
    ...
    "TEMPORARY_PASSAGE_EAST_MOUTH",
    "TEMPORARY_PASSAGE_WEST_MOUTH_EASTBOUND",
    "TEMPORARY_PASSAGE_WEST_MOUTH_WESTBOUND",
}
...
def contains_private_key(value: Any) -> bool:
    if isinstance(value, dict):
        return any(
            str(key).lower() in FORBIDDEN_PRIVATE_KEYS
            or contains_private_key(child)
...
def evaluate(
    ...
    for observation in observations:
        ...
        if not isinstance(sign_findings, list):
            raise FieldReviewError(
    ...
    if (
        ...
        legal_direction == "NO_MOTOR_ACCESS"
        and exit_movement != "PROHIBITED"
    ):
        blockers.append("FIELD_CONCLUSIONS_CONFLICT")
    if not isinstance(conclusions.get("reviewed_by"), str) or not conclusions[
        "reviewed_by"
    ].strip():
    ...
    return {
        ...
        "raw_evidence_file_count": (
            len(raw_hash_set) if raw_hash_set else 0
        ),
