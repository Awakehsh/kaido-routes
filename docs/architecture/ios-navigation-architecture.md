...
map-adjacent native credit in the parked chrome and never grants
navigation authority. The previous C2 and K7
...
12. Keep recommendation and customization in the same route-choice stage.
    ...
    authority.
13. Separate route selection from drive start with one parked journey
    ...
    synthetic journey. The review offers exactly one way to drive in a shipped
    build: deterministic playback and every launch-argument preview host
    compile under `#if DEBUG`, so a Release binary reaches the driving surface
    only through Core Location.
