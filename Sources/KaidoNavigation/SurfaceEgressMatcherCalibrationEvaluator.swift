...
public enum SurfaceEgressMatcherCalibrationEvaluatorError:
...
public enum SurfaceEgressMatcherCalibrationEvaluator {
  public static func evaluate(
  ...
  ) throws -> SurfaceEgressMatcherCalibrationReport {
    guard configuration.isValid else {
      throw SurfaceEgressMatcherCalibrationEvaluatorError.invalidConfiguration
    }
    guard !reportID.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
      throw SurfaceEgressMatcherCalibrationEvaluatorError.emptyReportID
    }
    guard let scope = traces.first?.context.scope else {
      throw SurfaceEgressMatcherCalibrationEvaluatorError.invalidTrace([
        "no surface egress private traces supplied"
      ])
    }
    ...
    var issues: [String] = []
    for trace in traces {
      issues.append(contentsOf: trace.validationIssues)
      if trace.context.scope != scope {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.mixedCalibrationScope
      }
      if trace.context.collectionMethod != collectionMethod {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.mixedCollectionMethod
      }
    }
    ...
    var entriesByID: [String: SurfaceEgressPrivateTraceEntry] = [:]
    for entry in entries {
      guard entriesByID.updateValue(entry, forKey: entry.observationID) == nil else {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.duplicateObservationID(
          entry.observationID
        )
      }
    }

    var annotationsByID: [String: SurfaceEgressGroundTruthAnnotation] = [:]
    for annotation in annotations {
      guard
        !annotation.observationID.isEmpty,
        !annotation.directedEdgeID.isEmpty,
        !annotation.occurrenceID.isEmpty,
        scope.occurrences.contains(where: {
          $0.occurrenceID == annotation.occurrenceID
            && $0.directedEdgeID == annotation.directedEdgeID
        })
      else {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.invalidAnnotation(
          annotation.observationID
        )
      }
      guard let entry = entriesByID[annotation.observationID] else {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.unknownAnnotatedObservation(
          annotation.observationID
        )
      }
      guard entry.status == .matched else {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.annotationRequiresMatchedEntry(
          annotation.observationID
        )
      }
      guard annotationsByID.updateValue(annotation, forKey: annotation.observationID) == nil
      else {
        throw SurfaceEgressMatcherCalibrationEvaluatorError.duplicateAnnotation(
          annotation.observationID
        )
      }
    }
    ...
    let annotatedEntries = matchedEntries.compactMap { entry -> AnnotatedEntry? in
      guard
        let annotation = annotationsByID[entry.observationID],
    ...
    let unsafeHighConfidenceOccurrenceCount = annotatedEntries.filter {
      $0.estimate.confidence == .high
        && $0.estimate.occurrenceID != $0.annotation.occurrenceID
    }.count
    ...
    let pipelineDurations = matchedEntries.compactMap { entry -> Int? in
      guard let matchingDuration = entry.matchingDurationMicroseconds else {
        return nil
      }
    ...
    return SurfaceEgressMatcherCalibrationReport(
      reportID: reportID,
      scope: scope,
      ...
      adapterRejectionCount: entries.filter {
        $0.status == .adapterRejected
      }.count,
      matcherRejectionCount: entries.filter {
        $0.status == .matcherRejected
      }.count,
      ...
      reliabilityBins: ReliabilityAccumulator(entries: annotatedEntries).bins,
...
private struct AnnotatedEntry {
  let estimate: SurfaceEgressMatcherEstimate
...
private struct ReliabilityAccumulator {
  ...
  init(entries: [AnnotatedEntry]) {
    ...
    for entry in entries {
      ...
      if entry.estimate.occurrenceID == entry.annotation.occurrenceID {
