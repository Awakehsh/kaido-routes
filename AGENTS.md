...
- The shared-snapshot Valhalla translator, checksummed build-manifest validator, response normalizer, bounded provider, and URLSession transport are implemented. A private Shinjuku build now proves complete Japan/Tokyo admin lookup and left-side driving while keeping raw OSM/provider artifacts ignored. Valhalla remains a bounded surface provider/oracle; live HTTP operations, data-distribution review, broader coverage, and field evidence are still pending.
