...
The repository is in **active product delivery**: contracts (`docs/`), machine-readable scenarios (`e2e/`), platform-light Swift modules (`Sources/`), a real iPhone app target (`KaidoRoutesApp`, generated with XcodeGen), and a bundled whole-Shuto network snapshot joined to dated operator facts. The shipped driving flow is an explicitly labeled simulation; no live-traffic, toll, or field-navigation authority is claimed. Do not describe the repository as feasibility-only, and do not treat the retained C2/K7 fixtures as delivery tracks.
