...
- `KaidoProductNavigationRuntime` keeps the validated product release, released Route Atlas, and exact navigation session composition together. It accepts no caller-supplied RoutePlan, corridor, DecisionZone, or guidance override. The current navigation artifact does not yet distribute entry-transition, recovery, or egress policy, so those behaviors stay unavailable rather than being invented by the runtime. KR-D26 proves that a failed joint release cannot create even a partial runtime identity.
