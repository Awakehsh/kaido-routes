...
- `KaidoPresentation` is the platform-light projection boundary. Phone, CarPlay, and voice values must come from one `NavigationSnapshot`; adapters never own occurrence progress. Preserve the exact Japanese sign and route shield in every locale, keep UI and voice locale independent, render LOW/projected positions conservatively, never color realtime-unconfirmed passage as open, expose no moving-time route editing, and name a compiled Finish drive exit before branch guidance. KR-U04 through U08 and U10 through U12 are seeded semantic contracts, not proof of SwiftUI layout, accessibility, CarPlay entitlement, audio lifecycle, or real hardware.
